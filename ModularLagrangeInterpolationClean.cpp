/*
Same as the other version in the library, except it is cleaner because of use of add, sub and mult functions
		...
		for(int j = 0; j < n; j++) {
			if(j != i) {
				...
				for(int k = 0; k < n - 1; k++)
					l_new[k + 1] = l[k];
				for(int k = 0; k < n; k++)
					l_new[k] = sub(l_new[k], mult(x[j], l[k]));
				swap(l_new, l);
			}
		}
		long long denomInv = 0;
		for(int j = 0; j < n; j++)
			denomInv = add(denomInv, mult(binpow(x[i], j), l[j]));
		denomInv = inverse(denomInv);
		for(int j = 0; j < n; j++) {
			l[j] = mult(l[j], mult(denomInv, y[i]));
			c[j] = add(c[j], l[j]);
		}
	}
